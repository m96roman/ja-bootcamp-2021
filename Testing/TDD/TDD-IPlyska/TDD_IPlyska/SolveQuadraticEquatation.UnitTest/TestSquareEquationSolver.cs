...
namespace SolveQuadraticEquatation.UnitTest
{
    ...
    public class TestSquareEquationSolver
    {
        ...
        public void Solve_GivenIvalidData_ShouldThrowExeption(double a, double b, double c)
        {
            //arrange
            //act assert
            Assert.That(async () => await equation.SolveAsync(a, b, c), Throws.TypeOf<ArgumentException>().With.Property("Message").EqualTo("Arguments can not be equals zero."));
        }

        [TestCase(0, 3, 0, 0)]
        [TestCase(2, 0, 0, 0)]
        public async Task Solve_Given_Two_Zero_Values_Should_Have_One_RootAsync(double a, double b, double c, double expectedValue)
        {
        ...
        public async Task Solve_Given_A_Zero_Should_Have_One_RootAsync(double a, double b, double c, double expectedValue)
        {
            //arrange
            //act
            EquationRoots roots = await equation.SolveAsync(a, b, c);
        ...
        public async Task Solve_Given_C_Zero_Have_Two_RootsAsync(double a, double b, double c, double expectedValue)
        {
            //act
            EquationRoots roots = await equation.SolveAsync(a, b, c);
            //assert
        ...
        public async Task Solve_Given_B_Zero_One_Way_With_RootsAsync(double a, double b, double c, double expectedValue)
        {
            //act
            EquationRoots roots = await equation.SolveAsync(a, b, c);
            //assert
        ...
        public async Task Solve_If_Discriminant_More_Than_ZeroAsync(double a, double b, double c, double r1, double r2)
        {
            //act
            EquationRoots roots = await equation.SolveAsync(a, b, c);
            //assert
        ...
        {
            //arrange
            Assert.That(async () => await equation.SolveAsync(a, b, c), Throws.TypeOf<NoRootsException>());
        }
        ...
        {
            //assert
            Assert.That(() => equation.SolveAndSaveSolutionAsync(a, b, c, filePath), Throws.TypeOf<ArgumentException>());
        }

        [TestCase(2, 4, 2, "<Root #1: -1>")]
        [TestCase(-5.6, 2.22, 1.24, "<Root #1: -9,7965730599; Root #2: 22,2285730599>")]
        [TestCase(0, 0, 3, "<No solution>")]
        public async Task Save_Result_Check_Formatting_For_Output_ResultAsync(double a, double b, double c, string content)
        {
            //arrange
